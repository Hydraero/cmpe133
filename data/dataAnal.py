...
for col in col_remove:
    df = df.drop(col, 1)
df = df.drop("parent_incident_type", 1)
df["weight"] = 1

df.to_csv ('allCrime.csv', index = None, header=True)
print('l')
...
# parkingDF = pd.DataFrame(pd.read_csv("ParkingViolations.csv"))
# parkingDF["weight"] = 1
# parkingDF.to_csv("ParkingViolations.csv", index = None, header = True)
